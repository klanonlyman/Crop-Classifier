...
def process(trainpath,savepath,csvname,size):
    ...
    for i in range(0,len(df)):
        ...
        img=cv2.imread(path)
        scale=2
        CW=img.shape[1]//scale
        CH=img.shape[0]//scale
        X=sub["target_x"]*-1
        Y=sub["target_y"]*-1
        pointX=CW+X
        pointY=CH+Y
        if pointY-(CH//scale)<0:
            starty=0
        else:
            starty=pointY-(CH//scale)

        if pointX-(CW//scale)<0:
            startx=0
        else:
            startx=pointX-(CW//scale)
            
        if X!=0 or Y!=0:
            img=img[starty:pointY+(CH//scale),startx:pointX+(CW//scale),:]
        else:
            img=img[CH-(CH//scale):CH+(CH//scale),CW-(CW//scale):CW+(CW//scale),:]
        try:
